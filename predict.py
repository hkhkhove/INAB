...
if "__main__"==__name__:
    ...
    config.path['log_file']=os.path.join(config.path['base_dir'],'log',f'predict_{time.strftime("%Y%m%d%H%M%S", time.localtime())}.txt')
    ...
    if args.input_dir is not None:
        pdbs=[os.path.join(args.input_dir,e) for e in os.listdir(args.input_dir) if e.endswith('.pdb') or e.endswith('.cif')]
    ...
    for pdb in tqdm(success_pdbs,desc="6.Predicting"):
        ...
        with open(pdb.replace('.pdb','_time.txt'),'w') as f:
            f.write(f"feats:{feats_end-feats_start:.2f}s\npredict:{predict_end-feats_end:.2f}s\n")
