...
def run(pdb_files):
    for i,pdb_file in enumerate(pdb_files):
        ...
        subprocess.run(['python','predict.py',
                        '--pdb_path',pdb_file,
                        '--model_path',f'./model_parameters/INAB_{category}.pth',
                        ])
...
if __name__=="__main__":
    ...
    pdb_files=[os.path.join('./dataset/efficiency_test/features',e) for e in os.listdir('./dataset/efficiency_test/features') if e.endswith('.pdb')]
    pdb_files=[pdb_file for pdb_file in pdb_files if not os.path.isfile(pdb_file.replace('.pdb','_output.txt'))]
