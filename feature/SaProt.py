...
def run(pdb_files, path):
    ...
    for pdb_file in tqdm(pdb_files, desc="4.SaProt"):
        ...
        save_path = pdb_file.replace(ext, "_saprot.npy")
        ...
        if oom:
            with open(path["log_file"], "a") as f:
                f.write(
                    f">{prot_name}(length={len(seq)}): SaProt feature processed using CPU due to CUDA OOM.\n"
                )
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        description="Extract SaProt representations from PDB files"
    )
    ...
    path["log_file"] = path.get("log_file") or os.path.join(
        path["base_dir"],
        "log",
        f"{time.strftime('%Y%m%d%H%M%S', time.localtime())}_saprot.txt",
    )
