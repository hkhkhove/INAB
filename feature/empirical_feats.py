...
def parallel(num_processes, pdb_files, func):
    with mp.Pool(num_processes) as pool:
        ...
        pbar=tqdm(total=len(pdb_files),desc=f"1.Empirical features({func.__name__})",leave=False)
        for _ in results:
            pbar.update()
...
if __name__ == '__main__':
    ...
    """
    import argparse
    argparser=argparse.ArgumentParser(description="Extract empirical features from PDB files")
