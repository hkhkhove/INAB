...
def get_atom_feats(pdb_file):
    ...
    atom_fea_dict = {'A': A, 'V': V, 'F': F, 'P': P, 'L': L, 'I': I, 'R': R, 'D': D, 'E': E, 'S': S,
...
def combine(pdb_files):
    ...
    for pdb_file in tqdm(pdb_files,desc="5.Combining features"):
        ...
        try:
            ...
            assert hhm.shape[0]==pssm.shape[0]==dssp.shape[0]==atom_feats.shape[0], f"feature dimension mismatch, hhm:{hhm.shape[0]},pssm:{pssm.shape[0]},dssp:{dssp.shape[0]},atom_feats:{atom_feats.shape[0]}"
            ...
            assert esm2_rep.shape[0]==gearnet_rep.shape[0]==saprot_rep.shape[0]==len(coords)==hhm.shape[0], f"feature dimension mismatch, esm2_rep:{esm2_rep.shape[0]},gearnet_rep:{gearnet_rep.shape[0]},saprot_rep:{saprot_rep.shape[0]},coords:{len(coords)},hhm:{hhm.shape[0]}"

            node_feats=np.concatenate([hhm,pssm,dssp,atom_feats,esm2_rep,gearnet_rep,saprot_rep],axis=1)
...
def run(pdb_files,path):
    path['log_file'] = path.get('log_file') or os.path.join(path['base_dir'], 'log', f"{time.strftime('%Y%m%d%H%M%S', time.localtime())}.txt")
...
if __name__ == '__main__':
    ...
    pdb_files=[e for e in pdb_files if not os.path.isfile(e.replace('.pdb','_input.pkl'))]

    import config

    success_pdbs=run(pdb_files,config.path)

    print(f"Successfully extracted features from {len(success_pdbs)}/{len(pdb_files)} PDB files.")