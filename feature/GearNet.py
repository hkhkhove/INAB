...
class GearNetProteinDataset(data.ProteinDataset):
    ...
    def load_pdbs(self, pdb_files):
        for pdb_file in tqdm(pdb_files, desc="3.GearNet(Loading PDBs)", leave=False):
            ...
            if not mol:
                print(
                    f"Can't construct molecule from pdb file `{pdb_file}`. Ignore this sample."
                )
                return None
            # try:
            ...
            protein = data.Protein.from_molecule(
                mol, atom_feature=None, bond_feature=None
            )
            if not protein:
                print(
                    f"Can't construct protein from pdb file `{pdb_file}`. Ignore this sample."
                )
                continue
...
def run(pdb_files, path):
    ...
    for i, e in enumerate(dataset):
        prot_name, ext = os.path.splitext(os.path.basename(pdb_files_[i]))

        save_path = pdb_files_[i].replace(ext, "_gearnet.npy")
        ...
        if oom:
            with open(path["log_file"], "a") as f:
                f.write(
                    f">{prot_name}(length={len(protein)}): GearNet feature processed using CPU due to CUDA OOM.\n"
                )
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(
        description="Extract GearNet representations from PDB files"
    )
    ...
    path["log_file"] = path.get("log_file") or os.path.join(
        path["base_dir"],
        "log",
        f"{time.strftime('%Y%m%d%H%M%S', time.localtime())}_gearnet.txt",
    )
