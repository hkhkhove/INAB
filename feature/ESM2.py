...
def run(pdb_files,path):
    ...
    for pdb_file in tqdm(pdb_files,desc="2.ESM-2"):
        save_path=pdb_file.replace('.pdb','_esm2.npy')
        ...
        prot_name,_=os.path.splitext(os.path.basename(fasta_file))
        ...
        if oom:
            with open(path['log_file'],'a') as f:
                f.write(f">{prot_name}(length={len(seq)}): ESM2 feature processed using CPU due to CUDA OOM.\n")
...
if __name__ =="__main__":
    ...
    """
    import argparse
    argparser=argparse.ArgumentParser(description="Extract ESM2 representations from PDB files")
    ...
    pdb_files=[e for e in pdb_files if not os.path.isfile(e.replace('.pdb','_esm2.npy'))]
